...
def Song_str(S:Song) -> str:
    '''Returns a string containing the song's information in an easily
    readable format
    '''
    return '  {:2d}  {:34s}  {:3d}  {:2d} '.format(S.track, S.title, S.length, S.play_count)

def Album_str(A: Album) -> str:
    '''Reutrns a string containing that album's information  in an easily readable format
    '''
    string = ''
    album = '{} / {} / {} / {}'.format(A.id, A.artist, A.title, A.year)
    string = string + album + "\n"
    for i in A.songs:
        song = ''
        song = song + Song_str(i)
        string = string + song + "\n"
    return string
...
def Songdisplay_str(S: Songdisplay) -> str:
    ''' Returns a string containing that information in an easily readable form
    '''
    return '{} / {} / {} / {} / {} / {} / {}'.format(S.artist,
           S.a_title, S.year, S.track, S.s_title, S.length, S.play_count)
...
def unplayed_songs(LA: list) -> list:
    ''' Returns a list of Songdisplays, one for each song that has never been played
    '''
    new_list = all_Songdisplays(LA)
    new_songdisplay = []
    for i in new_list:
        if i.play_count == 0:
            new_songdisplay.append(i)
    return new_songdisplay
...
for i in unplayed_songs(MUSIC):
    print(length_from_songdisplay(i))
...
def Song_listening_time(S: Song) -> int:
    ''' Return listening time of the song
    '''
    return S.play_count * S.length

def Album_listening_time(A: Album) -> int:
    ''' Return listening time of the album
    '''
    total = 0
    for i in A.songs:
...
def favorite_album(LA: list) -> Album:
    ''' Returns the album that is the "favorite."
    '''
    maxNum = 0
    for i in LA:
        total_time = 0
        total_time += Album_listening_time(i)
        if total_time > maxNum:
            maxNum = total_time
            total = []
            total.append(i)
    return total[0]
...
def top_n(MC: 'list of Album', n: int, sort_key: str, boolean: bool) -> 'list of Songdisplay':
    ''' Produce the same result as top_n_played by calling
    '''
    list_of_Songdisplays = all_Songdisplays(MC)
    list_of_Songdisplays.sort(key=sort_key, reverse=boolean)
    return list_of_Songdisplays[:n]

print("test_list1")
test_list1 = top_n(MUSIC, 3, play_count_from_songdisplay, True)
for i in test_list1:
    print(Songdisplay_str(i))

print("test_list2")
test_list2 = top_n(MUSIC, 10, length_from_songdisplay, False)
for i in test_list2:
    print(Songdisplay_str(i))
...
def Album_20th_listening_time(A: Album) -> int:
    ''' Return the total listening time for the album before 2000
    '''
    total = 0
    if A.year < 2000:
        for i in A.songs:
...
def favorite_album2(LA: list, function: str) -> Album:
    ''' Returns the album that is the "favorite."
    '''
    maxNum = 0
    for i in LA:
        total_time = 0
        total_time += function(i)
        if total_time > maxNum:
            maxNum = total_time
            total = []
            total.append(i)
...
print(Album_str(favorite_album2(MUSIC, Album_20th_listening_time)))

print()
print("Part (d8)")
print()

def collection_search(MC: 'list of Album', search: str) -> 'list of Songdisplay':
    ''' Return a list of Songdisplay that contain the string
    '''
    list_of_Songdisplays = []
    new_list = all_Songdisplays(MC)
    for i in new_list:
        if search in i.artist:
            list_of_Songdisplays.append(i)
        elif search in i.a_title:
            list_of_Songdisplays.append(i)
        elif search in i.s_title:
            list_of_Songdisplays.append(i)
    return list_of_Songdisplays

for i in collection_search(MUSIC, 'Mountain'):
    print(Songdisplay_str(i))
