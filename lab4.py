    ...
    if num == 0:    #This is for f4
        ...
        for i in rl:
            if i.cuisine == string:
                rl_new.append(i.name)
        return  rl_new
    elif num == 1:  #This is for calling function from f7
        string = input("Please select the cuisine of restaurants: ")
        rl_new = []
        for i in rl:
            if i.cuisine == string:
        ...
        return average
    elif num == 2:  #This is for calling function from f8
        ...
        average = average_price(rl_new)
        return average
    elif num == 3:  #This is for calling function from f9
...
def select_cheaper(rl: Restaurant, num: int) -> Restaurant:
    rl_new = []
    if num == 0:    #This is for f5
        number = float(input("Please enter a price: "))
        for i in rl:
            if i.price < number:
                rl_new.append(i.name)
    elif num == 1:  #This is for calling function from f9
        for i in rl:
            if i.price < 15.00:
                rl_new.append(i.name)
    return rl_new
...
def four_multiples():
    ...
    assert isinstance(num, int), "It is not an integer"
    j = 0
    for i in range(4):
        print(j * num)
        j = j + 1
